...
/** Same-origin proxy (dev :3002, Cloudflare Pages Function in production). */
export function getBestdoriAssetProxyUrl(assetPath: string) {
...
/** @deprecated Use getBestdoriAssetProxyUrl */
export const getLive2DAssetProxyUrl = getBestdoriAssetProxyUrl;

export function getLive2DBuildDataProxyUrl(assetBundleName: string, region: BestdoriRegion = "jp") {
  return getBestdoriAssetProxyUrl(`${region}/live2d/chara/${assetBundleName}_rip/buildData.asset`);
}
...
export function getLive2DBundleAssetProxyUrl(
  ...
  return getBestdoriAssetProxyUrl(`${region}/live2d/chara/${bundle}_rip/${normalizedFile}`);
}
